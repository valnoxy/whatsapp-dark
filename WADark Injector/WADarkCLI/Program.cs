...
namespace WADarkCLI
{
    class Program
    {
        ...
        {
            // TODO: Inplement URL Protocol into CLI version
        ...
        {
            // Check if Node.js (npm) exist on Windows
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                if (!File.Exists(Path.GetPathRoot(Environment.SystemDirectory) + "Program Files\\nodejs\\npx.cmd"))
                {
                    Console.WriteLine("An error has occurred ...");
                    Console.WriteLine("node.js (npm) is not installed. Please install it first before running WADark!\n");
                    Environment.Exit(1);
                }
            }
        ...
        private static bool MainMenu()
        {
            ...
            try
            {
                ...
                Console.WriteLine("WADark [WA Desktop Version: {0}]", ver.waversion);
            }
            catch (Exception ex)
            {
        ...
        private static void ThemeSelection()
        {
            ...
            {
                var ver = _serialized_json_data<VersionData>(url);
                Console.WriteLine("WADark [WA Desktop Version: {0}]\n", ver.waversion);
            }
            catch (Exception ex)
            {
                Console.ForegroundColor = ConsoleColor.Red;
        ...
        private static void CheckSys()
        {
            ...
            // Check if node.js is installed on OSX
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
